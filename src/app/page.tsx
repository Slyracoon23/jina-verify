    ...
    <div className="grid grid-rows-[20px_1fr_20px] items-center justify-items-center min-h-screen p-8 pb-20 gap-16 sm:p-20 font-[family-name:var(--font-geist-sans)]">
      <main className="flex flex-col gap-[32px] row-start-2 items-center w-full max-w-2xl">
        <div className="flex flex-col items-center gap-4 w-full">
          <h1 className="text-2xl font-bold mb-2">Jina Reader URL Generator</h1>
          <p className="text-center mb-4 text-sm">
            Enter a URL to generate a Jina Reader link. Jina Reader converts any URL to LLM-friendly input.
          </p>
...
function JinaReaderForm() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    try {
      new URL(url);
      // Generate Jina Reader URL
      const readerUrl = `https://r.jina.ai/${url}`;
      setJinaUrl(readerUrl);
    } catch {
  ...
  const copyToClipboard = () => {
    navigator.clipboard.writeText(jinaUrl)
      .then(() => {
        setIsCopied(true);
      ...
      {jinaUrl && (
        <div className="mt-6 p-4 border border-solid border-black/[.08] dark:border-white/[.145] rounded-md bg-black/[.03] dark:bg-white/[.03]">
          <div className="flex justify-between items-center mb-2">
            <p className="text-sm font-medium">Your Jina Reader URL:</p>
            <button
